...
#     if P_id[0] == 'E':
#
#         domains, unique_domains, exons, text1, domainshtml, Text_nodes, text_edges, tran_name, gene_name, Ensemble_geneID, entrezID, gene_description, exons, droped1, droped2, trID, co_partners = info.get_protein_info2(
#             P_id)
#
...
#         pd_interaction = pd.DataFrame(data=[p_name, P2_id, ' ; '.join(DI_edges), ' ; '.join(lost_edges)],
#                                       index=['Protein name', 'NCBI gene ID', 'Retained DDIs', 'Lost DDIs'])
#
...
#         _, missing_domain, gene_name, Ensemble_geneID, entrezID, tb_transc, table_domains, number = ex.input_exon(
#             exon_ID)
#
...
#                     N.append("{id: \"" + node + "\", label:  \"" + label + "\", group:  \"" + tr.group_node(node,
#                                                                                                             entrezID) + "\", physics:" + tr.physics(
#                         node, entrezID) + ", source:  \"" + tr.source_node(node, entrezID,
#                                                                            protein_with_DDI) + "\", value:  \"" + tr.value_node(
#                         node, entrezID) + "\"},")
#
#                 except KeyError:
#                     print(node)
...
#
#                 N.append(
#                     "{id: \"" + node + "\", label:  \"" + label + "\", " + color + "group:  \"" + tr.group_node(node,
#                                                                                                                 entrezID) + "\", physics:" + tr.physics(
#                         node, entrezID) +
#                     ", source:  \"" + tr.source_node(node, entrezID,
#                                                      protein_with_DDI) + "\", value:  \"" + tr.value_node(node,
#                                                                                                           entrezID) + "\"},")
#
#     for e in g.edges():
#         gene1 = e[0].split('/')[0]
...
#             E.append("{from: \"" + e[0] + "\", to: \"" + e[1] + "\", dashes:  " +
#                      tr.edge_dashes(e, entrezID, missing_domain)[0] + "," + tr.edge_option(e, entrezID,
#                                                                                            co_partners) + "},")
#
#     return N, E, len(g) - 1
